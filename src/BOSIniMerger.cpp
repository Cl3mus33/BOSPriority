...
// previous output as a new source mod. Deliberately still says "BOSPriority" even after the app
// was renamed to INIPriority: this exact text is already sitting inside real users' output files
// from earlier releases, and scan()'s check is an exact/starts_with match - changing it would make
// existing installs stop recognizing their own already-blanked files on upgrade.
...
{
    // IniDiscovery::findFiles already returns *_swap.ini alphabetically (BOS's own default order,
    // case-sensitive, last file wins) - this just layers BOS-specific filtering (skip this tool's
    // own previous output) on top.
    vector<fs::path> found;
    for (auto& path : IniDiscovery::findFiles(gameDir, L"_swap")) {
        if (!isOwnOutput(path)) {
