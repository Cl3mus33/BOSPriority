...
4. If any keys conflict, **Manage Conflicts...** opens a table - filter by type, select a
   ...
   conflict at once instead of resolving them one by one - saved to the output folder and
   reapplied automatically on every future scan, including against conflicts that don't exist yet.
